        ...
        <div class="card card-container h-100 d-flex flex-column">
            ...
            <div class="card-actions-sidebar">
                <button type="button"
                    ...
                    data-id="{{ $package->id }}" data-active="{{ $package->is_active }}" data-bs-toggle="tooltip"
                    data-bs-custom-class="{{ $package->is_active ? 'tooltip-secondary' : 'tooltip-success' }}"
                    data-bs-placement="top" title="{{ $package->is_active ? 'إلغاء التفعيل' : 'تفعيل الباقة' }}">
                    <i class="{{ $package->is_active ? 'ri-close-circle-line' : 'ri-checkbox-circle-line' }}"></i>
                </button>

                <button type="button"
                    class="btn btn-icon {{ $package->coming_soon ? 'btn-outline-warning' : 'btn-outline-primary' }} toggle-package-btn-hidden"
                    data-id="{{ $package->id }}" data-active="{{ $package->coming_soon }}" data-bs-toggle="tooltip"
                    data-bs-custom-class="{{ $package->coming_soon ? 'tooltip-warning' : 'tooltip-primary' }}"
                    data-bs-placement="top" title="{{ $package->coming_soon ? 'إخفاء الباقة' : 'إظهار الباقة' }}">
            ...
            <div class="card-body d-flex flex-column">
                <div class="p-1 flex-grow-1 d-flex flex-column">
                    <div class="d-flex justify-content-between align-items-start flex-wrap mb-3">
                        <h5 class="fw-bold mb-0" style="word-break: break-word; white-space: normal; max-width: 60%;">
                            {{ $package->t('name') }}
                        </h5>
                        <span
                        ...
                        <div class="row text-center mb-3">
                            ...
                            <div class="col-6 col-md-3 mb-3 mb-md-0">
                                <div class="fs-18 fw-bold mb-1">
                                    <i class="ri-stack-line me-1"></i>{{ $package->product_count }}
                                </div>
                            ...
                            <div class="col-6 col-md-3">
                                <div class="fs-18 fw-bold mb-1">
                                    <i class="ri-gift-line me-1 text-primary"></i>{{ $package->free_product_count }}
                                </div>
                            ...
                            <li class="d-flex align-items-center mb-2">
                                <span class="me-2">
                                    <i class="ri-checkbox-circle-line fs-2 toggle-feature-icon {{ $feature->is_active ? 'text-success' : 'text-muted' }}"
                                        data-id="{{ $feature->id }}" data-active="{{ $feature->is_active ? 1 : 0 }}"
