...
@section('content')
    <div class="container-fluid px-5 py-3">
        <!-- Page Header -->
        <x-breadcrumb.breadcrumb title="{{ __('dashboard.footer') }}" :breadcrumbs="[['name' => __('dashboard.footer'), 'route' => 'footer.index']]" />
        <!-- Page Header Close -->
        ...
        <x-cards.page-card>
            ...
            <x-form.form-component :route="route('footer.store')" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <x-input.input-field name="all[email]" label="{{ __('dashboard.email') }}"
                            placeholder="{{ __('dashboard.email') }}" :value="$content['all']['email'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                        <x-input.input-field name="all[phone]" label="{{ __('dashboard.phone') }}"
                            placeholder="{{ __('dashboard.phone') }}" :value="$content['all']['phone'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                        <x-input.input-field name="all[whatsapp]" label="{{ __('dashboard.whatsapp') }}"
                            placeholder="{{ __('dashboard.whatsapp') }}" :value="$content['all']['whatsapp'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                        <x-input.input-field name="all[facebook_link]" label="{{ __('dashboard.facebook_link') }}"
                            placeholder="{{ __('dashboard.facebook_link') }}" :value="$content['all']['facebook_link'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                        <x-input.input-field name="all[instagram_link]" label="{{ __('dashboard.instagram_link') }}"
                            placeholder="{{ __('dashboard.instagram_link') }}" :value="$content['all']['instagram_link'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                        <x-input.input-field name="all[twitter_link]" label="{{ __('dashboard.twitter_link') }}"
                            placeholder="{{ __('dashboard.twitter_link') }}" :value="$content['all']['twitter_link'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <x-input.input-field name="website_name[en]" label="{{ __('dashboard.website_name_en') }}"
                            placeholder="{{ __('dashboard.website_name_en') }}" :value="$content['website_name']['en'] ?? ''" />
                    </div>

                    {{-- Image Upload --}}
                    <div class="col-md-6">
                        <x-input.input-field name="image" type="file" label="{{ __('dashboard.website_logo') }}" />
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <x-editor.summernote id="content_ar" label="{{ __('dashboard.content_ar') }}" name="content[ar]"
                            :value="$content['content']['ar'] ?? ''" />
                    </div>
                    <div class="col-md-6">
                        <x-editor.summernote id="content_en" label="{{ __('dashboard.content_en') }}" name="content[en]"
